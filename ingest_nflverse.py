...
def ingest_season_stats(seasons: list[int], db_path: str = DB_PATH) -> int:
    ...
    with connect(db_path) as conn:
        ...
        for _, r in df.iterrows():
            ...
            row["fumbles_lost"] = sum(
                _none_if_nan(r.get(c)) or 0
                for c in (
                    "sack_fumbles_lost",
...
def ingest_all(seasons: list[int], db_path: str = DB_PATH) -> dict:
    """Run rosters -> season -> weekly in order. Rosters first so the
    crosswalk is populated before stats try to look up player_id."""
    init_db(db_path)
    return {
...
def main():
    ...
    p.add_argument(
        "--step",
        choices=["all", "rosters", "season", "weekly"],
        default="all",
    )
